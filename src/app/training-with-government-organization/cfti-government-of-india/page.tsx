...
export default async function CftiGovernmentOfIndiaPage() {
  ...
  try {
    await connectDB();
    const dbImages = await GovTrainingImage.find({ org: "CFTI" })
  ...
  const publicDir = path.join(process.cwd(), "public", "training", "CFTI");
  let localImages: string[] = [];
  try {
    if (fs.existsSync(publicDir)) {
      localImages = fs.readdirSync(publicDir)
        .filter(file => file.match(/\.(jpg|jpeg|png|gif)$/i))
        .map(file => `/training/CFTI/${file}`);
    }
  } catch (error) {
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-5xl">
        <h2 className="text-3xl font-extrabold text-center text-slate-900 mb-10">CFTI Government of India</h2>
        <div className="mt-8">
          <ImageGallery images={images} basePath="/training/CFTI" />
        </div>
      </div>
    </InternalPageLayout>
  );
